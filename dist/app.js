...
class FrameworkClient {
    ...
    async loadBotCommands(path, mask) {
    ...
    async handleCommand(commandString, message, commandsList = this.botCommands, event, cmdDpth = 1) {
        ...
        if (command) {
            ...
            if (isMultiCommand(command)) {
                ...
                if (!subCommandStr || subCommandStr == command.name || command.altNames?.includes(subCommandStr))
    ...
    async execCommand(command, message, event) {
        try {
            ...
            // @ts-ignore
            const args = await ArgumentParser.instance
                // @ts-ignore
    ...
    async checkUserPerm(command, message, hideErrors = false) {
