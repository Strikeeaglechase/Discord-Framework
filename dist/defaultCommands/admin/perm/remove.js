...
class PermRemove extends Command {
    ...
    async run(event, permName, target) {
        ...
        await event.framework.utils.reactConfirm(`Are you sure you would like to remove the __${event.perm.name}__ permission from ${target.value.toString()}?`, event.message, {
            onConfirm: async () => {
                assert(!Array.isArray(event.perm), `Perms cannot be an array`);
