...
function getRows(options) {
    ...
    const rows = new Array(requiredRows).fill(null).map(_ => new ActionRowBuilder());
    options.forEach((option, idx) => {
        const rowIdx = Math.floor(idx / 5);
        rows[rowIdx].addComponents(new ButtonBuilder({
...
async function getButtonSelect(framework, channel, userId, prompt, options) {
    const rows = getRows(options);
    const message = await channel.send({ embeds: [prompt], components: rows });
    async function updateButtons(options) {
